...
func (t *Tui) searchPrompt() {
	...
	case "edit":
		t.state.edit.ResizeItem(t.state.command, 3, 1)
	}
...
func (t *Tui) nextPanel() {
	...
	case "edit":
		idx := (t.state.editPanels.currentPanel + 1) % len(t.state.editPanels.panel)
		t.switchPanel(t.state.editPanels.panel[idx].name())
	}
}
