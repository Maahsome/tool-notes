...
func fetchToolList(dir_path string) []string {
	files := []string{}

	filepath.Walk(dir_path, func(path string, f os.FileInfo, err error) error {

		if !strings.HasPrefix(path, ".") && strings.HasSuffix(path, ".yaml") {
...
func listTools() {
	...
	tool_list := fetchToolList(toolPath)
	logrus.Info(fmt.Sprintf("%s", tool_list))
}
